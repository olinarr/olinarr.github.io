...
class Entry:
    def __init__(self, entry, isPreprint):
        ...
        self.pdf = os.path.isfile(
            "../files/publications/pdf/" + self.file_name + ".pdf"
        )

        self.poster = os.path.isfile(
            "../files/publications/posters/" + self.file_name + ".pdf"
        )
...
for key in ("pubs", "preprints"):
    entries[key] = [
        Entry(entry=e, isPreprint=(key == "preprints")) for e in entries[key]
    ]
...
sorted_years = ["preprints", "forthcoming"] + sorted(
...
for year in sorted_years:
    ...
    for entry in sorted(year_entries):
        ...
        if entry.pdf:
            resources.append(
                '<a target="_blank" href="./files/publications/pdf/'
                + entry.file_name
                + '.pdf">pdf</a>'
            )
        ...
        if entry.poster:
            resources.append(
                '<a target="_blank" href="./files/publications/posters/'
                + entry.file_name
                + '.pdf">poster</a>'
            )
        ...
        resources.append(
            '<a target="_blank" href="./files/publications/bib/'
            + entry.file_name
            + '.bib">bib</a>'
        )
